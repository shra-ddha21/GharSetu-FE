...
const ProtectedRoute = ({ allowedRoles }) => {
  ...
  if (loading) return <div className="p-8 text-center font-medium text-slate-500">Loading...</div>;
    ...
    <div className="min-h-screen bg-slate-50 flex font-sans text-slate-900 border-slate-200">
      ...
      <aside className="w-64 bg-white border-r border-slate-200 flex flex-col p-6 shrink-0">
        <div className="flex items-center gap-3 mb-10">
          ...
          <span className="text-xl font-bold tracking-tight text-slate-800">GharSetu</span>
        </div>
        ...
        <div className="mt-auto">
          <div className="p-4 bg-slate-900 rounded-2xl text-white">
            <p className="text-xs text-slate-400 mb-1 font-medium uppercase tracking-wider">Active {user?.role}</p>
            <p className="text-sm font-semibold truncate">{user?.name || user?.businessName || user?.email}</p>
            <button
              onClick={logout}
      ...
      <BrowserRouter>
        <Routes>
          ...
          <Route element={<ProtectedRoute allowedRoles={['admin']} />}>
            <Route element={<SidebarLayout links={[
          ...
          <Route element={<ProtectedRoute allowedRoles={['user']} />}>
            <Route element={<SidebarLayout links={[
              ...
              { label: 'Search Providers', href: '/user/search', icon: Search },
          ...
          <Route element={<ProtectedRoute allowedRoles={['provider']} />}>
            <Route element={<SidebarLayout links={[
              ...
              { label: 'Incoming Requests', href: '/provider/incoming-requests', icon: Briefcase },
              { label: 'Assigned Requests', href: '/provider/assigned-requests', icon: FileText },
            ]} />}>
              <Route path="/provider/dashboard" element={<ProviderDashboard />} />
              <Route path="/provider/incoming-requests" element={<ProviderIncoming />} />
              <Route path="/provider/assigned-requests" element={<ProviderAssigned />} />
            </Route>
          </Route>
